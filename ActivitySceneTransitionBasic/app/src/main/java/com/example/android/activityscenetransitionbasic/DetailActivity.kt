...
class DetailActivity : AppCompatActivity() {
     ...
     * view to our layout file `R.layout.details`.
     *
     * We initialize our [ScrollView] variable `rootView` to the view with ID `R.id.root_view`
     ...
     *
     *
     * Next we initialize our [Item] field [mItem] by calling the [Item.getItem] static method to
     * retrieve the [Item] in the dataset [Item.ITEMS] which has the ID that was passed us as an
     * extra in the `Intent` that launched us stored under the key [EXTRA_PARAM_ID] ("detail:_id").
     * We then initialize our [ImageView] field [mHeaderImageView] by finding the view with ID
     * `R.id.imageview_header`, and initialize our [TextView] field [mHeaderTitle] by finding the
     * view with ID `R.id.textview_title`. We then set the name of [mHeaderImageView] to be used to
     * identify Views in Transitions to [VIEW_NAME_HEADER_IMAGE] ("detail:header:image"), and set
     * the name of [mHeaderTitle] to be used to identify Views in Transitions to
     * [VIEW_NAME_HEADER_TITLE] ("detail:header:title"). Finally we call our method [loadItem] to
     * have it set the text of [mHeaderTitle] and to call either [loadThumbnail] or
     * [loadFullSizeImage] as appropriate to load the correct image into [mHeaderImageView].
     *
